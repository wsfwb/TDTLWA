...
def main():
 src=list(csv.DictReader(open(E26/'evaluation/results/e26_canonical_predictions.csv'))); y=np.asarray([int(r['gold_label_evaluation_only']) for r in src]); names={'Historical Full':'historical_full_prediction','Reconstructed Full':'full_prediction','D3':'residual_prediction'}; rng=np.random.default_rng(20260822); rows=[]
 for typ,name in [('absolute','e31_absolute_winner'),('nontrivial','e31_nontrivial_winner'),('harm_protected','e31_harm_protected_winner')]:
 ...
 with open(OUT/'e31_bootstrap_summary.csv','w',newline='') as f:w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
 print({'comparisons':len(rows),'replicates':2000})
